...
static Varint readVarint(const uint8_t* buf, size_t len, size_t pos) {
    ...
    if (pos >= len)
        throw std::runtime_error("varint out of bounds");
...
std::vector<uint8_t> readPage(std::ifstream& f, uint32_t pageNo, size_t pageSize) {
    if (pageNo < 1)
        throw std::runtime_error("pageNo starts at 1");
...
static ColumnValue decodeValue(const uint8_t* body, size_t bodyLen, size_t& pos, uint64_t serialType) {
    ...
    if (serialType == 1) { // 1-byte signed int
        if (pos + 1 > bodyLen)
            throw std::runtime_error("int1 OOB");
    ...
    if (serialType == 2) { // 2-byte signed int
        if (pos + 2 > bodyLen)
            throw std::runtime_error("int2 OOB");
        out.type = ColType::Int;
    ...
    if (serialType == 3) { // 3-byte signed int
        if (pos + 3 > bodyLen)
            throw std::runtime_error("int3 OOB");
        out.type = ColType::Int;
    ...
    if (serialType == 4) { // 4-byte signed int
        if (pos + 4 > bodyLen)
            throw std::runtime_error("int4 OOB");
        out.type = ColType::Int;
    ...
    if (serialType == 5) { // 6-byte signed int
        if (pos + 6 > bodyLen)
            throw std::runtime_error("int6 OOB");
        out.type = ColType::Int;
    ...
    if (serialType == 6) { // 8-byte signed int
        if (pos + 8 > bodyLen)
            throw std::runtime_error("int8 OOB");
        out.type = ColType::Int;
    ...
    if (serialType == 7) { // 8-byte float
        if (pos + 8 > bodyLen)
            throw std::runtime_error("float8 OOB");
    ...
    if (serialType >= 12) {
        ...
        if (pos + nBytes > bodyLen)
            throw std::runtime_error("blob/text OOB");
        if (isText) {
...
static Record parseRecord(const uint8_t* payload, size_t payloadLen) {
    ...
    if (hsz.bytes > payloadLen)
        throw std::runtime_error("header varint OOB");
    ...
    if (pos != headerEnd)
        throw std::runtime_error("record header size mismatch");
    ...
    if (bodyPos > payloadLen)
        throw std::runtime_error("record body overflow");
    return r;
}

// ---------- Page 2 table-leaf cell parsing ----------
int main() {
    const char* filename = "/mnt/c/Users/Manoj/Projects/codecrafters-sqlite-cpp/sample.db";
    ...
    BtreeHeader h = parseBtreeHeader(page, /*isPage1=#1#false);
